...
exports.uploadEvent = async (req,res,next) =>{
    try {
        const {title, description, addedAt,host} = req.body;

        const base64Data = await Imagetobase64(req.files.eventImage.path);
        //console.log("base64Data", base64Data);

        const event = await Event.create({
            title, description, host, addedAt,eventImage: `data:${req.files.eventImage.type};base64,${base64Data}`, addedAt: Date.now()
        }).save();
